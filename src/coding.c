...
double FisherCoding(int atx, int aty, int size, int *xd, int *yd, int *is, int *qalf, int *qbet)
{
	int x, y, qalfa, qbeta, i, j;
...
double HurtgenCoding(int atx, int aty, int size, int *xd, int *yd, int *is, int *qalf, int *qbet)
{
	int x, y, qalfa, qbeta, i, j;
...
double Mc_SaupeCoding(int atx, int aty, int size, int *xd, int *yd, int *is, int *qalf, int *qbet)
{
	int x, y, qalfa, qbeta, i, j;
	int counter, found = 0;
	...
	for (isom = IDENTITY; isom <= S_DIAGONAL; isom++)
	{
		...
		while (1)
		{ /* To make sure there is at least one kd-tree searched  */
		...
		for (a = (cx + n_p_class - r) % n_p_class, aa = 0; aa < t; a = (a + 1) % n_p_class, aa++)
			for (b = (cy + n_p_class - r) % n_p_class, bb = 0; bb < t; b = (b + 1) % n_p_class, bb++)
			{
				...
				if (class_polar_saupe[tip][a][b] != NULL)
					found = kdtree_search(r_vector_saupe, f_vect[tip][a][b], feat_vect_dim[tip], class_polar_saupe[tip][a][b], eps, matches, nlist);
...
double MassCenterCoding(int atx, int aty, int size, int *xd, int *yd, int *is, int *qalf, int *qbet)
{
	int x, y, qalfa, qbeta, i, j;
	...
	for (isom = IDENTITY; isom <= S_DIAGONAL; isom++)
	{
		...
		while (1)
		{ /* To make sure there is at least one list searched */
			for (a = (cx + n_p_class - r) % n_p_class, aa = 0; aa < t; a = (a + 1) % n_p_class, aa++)
				for (b = (cy + n_p_class - r) % n_p_class, bb = 0; bb < t; b = (b + 1) % n_p_class, bb++)
					if (class_polar[tip][a][b] != NULL)
						goto out_loops;
...
double SaupeCoding(int atx, int aty, int size, int *xd, int *yd, int *is, int *qalf, int *qbet)
{
	int x, y, qalfa, qbeta, i, j;
...
double TaiCoding2(int atx, int aty, int size, int *xd, int *yd, int *is, int *qalf, int *qbet)
{
	int x, y, qalfa, qbeta, i, j;
	int tip, counter, isometry;
...
void quadtree(int atx, int aty, int size, double tol_entr, double tol_rms, double tol_var)
{
	double best_rms;
