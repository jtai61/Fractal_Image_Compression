...
int hurtgen_class(int size, double **block)
{
    ...
    mean = (quadrant_mean[0] + quadrant_mean[1] + quadrant_mean[2] + quadrant_mean[3]) / (size * size);
...
void dctclass(double dct_h, double dct_v, int *clas)
{
    double dct_h_magnitude = fabs(dct_h), dct_v_magnitude = fabs(dct_v);

    if (dct_h_magnitude < DCT_TS && dct_v_magnitude < DCT_TS)
    {
        *clas = 0;
    }
    else if (fabs(dct_h_magnitude - dct_v_magnitude) < DCT_TD)
    {
        *clas = 1;
    }
    else
...
void ComputeMcVectors(double **block, double **block_tmp, int size, int index, double *vector)
{
    double mean;
    double xx, yy;
    double theta;
    ...
    /* Compute the MC of the block and put it in polar coordinates */
    ComputeMc(block_1, newsize, &xx, &yy, 0);
    theta = atan2(yy, xx);
    if (theta >= 0.0)
        vector[0] = theta;
    else
    ...
    /* Compute other features by transforming the original block */
    for (i = 1; i < 2; i++)
    {
        mean = sum / (newsize * newsize);
        sum = 0.0;
        for (x = 0; x < newsize; x++)
            for (y = 0; y < newsize; y++)
            {
...
void ComputeBitmapHistVectors(double **block, int size, double mean, double *vector)
{
    int i, j, k;
    unsigned int sum;
...
void MassCenterIndexing(int size, int s)
{
    ...
    for (i = 0; i < image_height - 2 * size + 1; i += SHIFT)
    {
        ...
        printf(" Extracting features (Mass Center) domain (%dx%d)  %d \r", size, size, count);
        fflush(stdout);
    }
...
void Mc_SaupeIndexing(int size, int s)
{
    ...
    printf(" Extracting features (Saupe) domain (%dx%d) building Kd-trees", size, size);
    fflush(stdout);

    for (h = 0; h < n_p_class; h++)
        for (i = 0; i < n_p_class; i++)
        {
            c_book[s][h][i] = (struct code_book *)malloc(item_per_class[s][h][i] * sizeof(struct code_book));
...
void TaiIndexing(int size, int s)
{
    ...
    for (i = 0; i < image_height - 2 * size + 1; i += SHIFT)
    {
        for (j = 0; j < image_width - 2 * size + 1; j += SHIFT)
        {
            ...
            node->sum2 = sum2;
            node->iso = iso;
            node->next = class_tai[s][clas][dct_clas];
            class_tai[s][clas][dct_clas] = node;
        }
        printf(" Classification (Tai) domain (%dx%d)  %d \r", size, size, count);
        fflush(stdout);
    }
